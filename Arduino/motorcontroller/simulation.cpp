...
void Simulation::loop ()
{
   if (loopCount++ % 5250 == 0) {  // corresponds with a move time of 10 seconds for the full stroke
      if (movingForward || movingReverse)
      {
         ...
         int32_t incs = toIncrements (10.0);
         if (position > incs) {
            position = incs;
            if (!posWarningGiven) {
...
{
   return (position > toIncrements(LEPos + homeDistance))? true:false;

}

bool Simulation::isAbovePhysicalUpper ()
{
   return (position < toIncrements(UEPos - homeDistance))? true:false;
}
...
{
   return (toMeters(position) < UEPos)? true:false;
}

bool Simulation::isAboveUpperGoingDown ()
{
   return (toMeters(position) < (UEPos+backlash))? true:false;
}

bool Simulation::isBelowLowerGoingUp ()
{
   return (toMeters (position) > (LEPos-backlash)) ? true:false;
}

bool Simulation::isBelowLowerGoingDown ()
{
   return (toMeters(position) > (LEPos)) ? true:false;
}
...
bool Simulation::isAboveUpper ()
{
   ...
   if (b != oldAboveUpper) {
      ...
      DEBUG ("   Simulation: isAboveUpper became %s at absolute %f, relative %f [m]\n",
              b?"true":"false", toMeters (position), getPosition ());
   }
...
bool Simulation::isBelowLower ()
{
   ...
   if (b != oldBelowLower) {
      ...
      DEBUG ("   Simulation: isBelowLower became %s at absolute %f, relative %f [m]\n", 
                  b?"true":"false", toMeters (position), toMeters (getPosition ()));
   }
...
{ 
   offset += toIncrements(ofsChg);
   DEBUG (">< Sim: setOffset (%f); new offset = %f\n", ofsChg, toMeters (offset));
   return true;
}
...
Simulation *Simulation::getSimPtr () {return &sim;}
